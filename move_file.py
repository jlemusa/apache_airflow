...
with DAG(dag_id="moving_files", start_date=datetime(2023, 1, 1), schedule_interval="@once"):
    list_files_and_log = SSHOperator(task_id="list_files_and_log", command=f"find /Files/ -type f -iname file* > /LogProcess/log-{datetime.now().strftime('%Y%m%d%H%M%S')}", ssh_conn_id="SSH_to_SV02")
